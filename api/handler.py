"""
Hybrid handler.py that supports both HTTP API mode and normal RunPod serverless mode
- HTTP API mode: Uses SQLite job management with HTTP API server
- Serverless mode: Uses original RunPod serverless functionality
"""
...
HTTP_API_MODE = os.environ.get("HTTP_API_MODE", "false").lower() == "true"

# Conditionally import API server components only if in HTTP API mode
if HTTP_API_MODE:
...
# API Server configuration (only used in HTTP API mode)
if HTTP_API_MODE:
    API_HOST = os.environ.get("API_HOST", "0.0.0.0")
    API_PORT = int(os.environ.get("API_PORT", "8000"))
    DB_PATH = os.environ.get("SQLITE_DB_PATH", "/tmp/comfyui_jobs.db")

# ... (keeping all the original helper functions from handler.py)
...
def handler(job):
    """
    Main handler function - this is the original ComfyUI handler function.
    It now works with both the HTTP API and the original RunPod serverless system.
    """
    job_input = job["input"]
    job_id = job["id"]

    print(f"worker-comfyui - Processing job {job_id}")

    # Make sure that the input is valid
    validated_data, error_message = validate_input(job_input)
    if error_message:
        print(f"worker-comfyui - Job {job_id} validation failed: {error_message}")
    ...
    if errors:
        ...
        print(f"worker-comfyui - Job {job_id} completed with errors/warnings: {errors}")

    if not output_data and errors:
        print(f"worker-comfyui - Job {job_id} failed with no output images.")
        return {"error": "Job processing failed", "details": errors}
    elif not output_data and not errors:
        print(f"worker-comfyui - Job {job_id} completed successfully, but the workflow produced no images.")
        final_result["status"] = "success_no_images"
        final_result["images"] = []

    print(f"worker-comfyui - Job {job_id} completed. Returning {len(output_data)} image(s).")
    return final_result


def start_combined_server():
    """
    Start both the HTTP API server and the RunPod worker with SQLite job management.
    This is the main entry point for production HTTP API mode.
    """
    if not HTTP_API_MODE:
        raise RuntimeError("start_combined_server() called but HTTP_API_MODE is not enabled")
    
    print("worker-comfyui - Starting combined HTTP API and SQLite worker server...")
    
    # Configure SQLite database path
    if DB_PATH:
        from .job_manager import job_manager
        job_manager.db_path = DB_PATH
        job_manager._init_database()
        print(f"worker-comfyui - Using SQLite database: {DB_PATH}")
    
    # Start the HTTP API server in a separate thread
    print(f"worker-comfyui - Starting HTTP API server on {API_HOST}:{API_PORT}")
    ...
    print("worker-comfyui - Starting RunPod worker with SQLite job management...")
    start_worker_with_sqlite_jobs(handler)


def start_worker():
    """
    Start the worker in the appropriate mode based on environment variables.
    This is the main entry point that determines whether to use HTTP API mode or serverless mode.
    """
    if HTTP_API_MODE:
        print("worker-comfyui - HTTP_API_MODE=true detected, starting in HTTP API mode with SQLite...")
        start_combined_server()
    else:
        print("worker-comfyui - Starting in original RunPod serverless mode...")
